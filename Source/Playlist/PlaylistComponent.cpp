            ...
            for (auto* aut : t->automationClips) {
                if (aut && aut->isShowing) {
                    ...
                    if (aut->lane.nodes.empty()) {
                        ...
                        g.drawHorizontalLine((int)valY, 0.0f, (float)getWidth());
                    } else {
                        juce::Path p;
                        bool first = true;
                        // Simplificación: iteramos por píxeles o intervalos cortos
                        for (float tX = 0; tX <= getWidth(); tX += 3.0f) {
                            float realTime = (tX + (float)hS) / (float)hZoom;
                        ...
                        for (const auto& node : aut->lane.nodes) {
                            float screenX = (node.x * (float)hZoom) - (float)hS;
