...
#include "PlaylistComponent.h" // Incluimos el Playlist real para acceder a sus miembros
...
void PlaylistDropHandler::processExternalFiles(const juce::StringArray& files, int x, int y, PlaylistComponent& playlist) {
    ...
    if (tIdx != -1 && playlist.tracksRef && (*playlist.tracksRef)[tIdx]->getType() != requiredType) {
        tIdx = -1;
    }
    ...
    float absX = (x + (float)playlist.hNavigator.getCurrentRangeStart()) / playlist.hZoom;
...
void PlaylistDropHandler::processInternalItem(const juce::DragAndDropTarget::SourceDetails& dragSourceDetails, PlaylistComponent& playlist) {
    ...
    // Interceptamos si el archivo viene desde el FileBrowserPanel
    if (desc == "FileBrowserDrag") {
        if (auto* tree = dynamic_cast<juce::FileTreeComponent*>(dragSourceDetails.sourceComponent.get())) {
            ...
            if (f.existsAsFile()) {
                ...
                // Redirigimos al procesador de archivos externos
    ...
    // Lógica para arrastre de clips desde el Picker Panel (MIDI o Audio clonados)
    ...
    if (tokens.size() == 3) {
        ...
        if (tIdx == -1 && playlist.onNewTrackRequested) {
            playlist.onNewTrackRequested(requiredType);
            if (playlist.tracksRef && !playlist.tracksRef->isEmpty()) {
                tIdx = playlist.tracksRef->size() - 1;
            }
        }
        ...
        if (isMidi) {
            ...
            if (!sourceMidi && playlist.trackContainer) {
                for (auto* mc : playlist.trackContainer->unusedMidiPool) {
                    if (mc->name == itemName) { sourceMidi = mc; break; }
                }
            }
            ...
            if (!sourceAudio && playlist.trackContainer) {
                for (auto* ac : playlist.trackContainer->unusedAudioPool) {
                    if (ac->name == itemName) { sourceAudio = ac; break; }
                }
            }
