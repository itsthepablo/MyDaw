...
void PlaylistComponent::timerCallback() {
  // CORRECCI├ôN DE RACE CONDITION: Solo lee el motor si est├í en Play y NO se
  // est├í arrastrando el timeline
...
void PlaylistComponent::updateScrollBars() {
  ...
  // RESTAMOS 60px DE ALTURA PARA QUE LA ├ÜLTIMA PISTA NO QUEDE DETR├üS DEL MASTER
  // STRIP
...
void PlaylistComponent::addMidiClipToView(Track *targetTrack,
  ...
  if (targetTrack != nullptr)
    targetTrack
        ->commitSnapshot(); // DOUBLE BUFFER: clip MIDI a├▒adido desde Piano Roll
  updateScrollBars();
  repaint();
  hNavigator.repaint();
}
...
int PlaylistComponent::getTrackY(Track *targetTrack) const {
  ...
  // BUG FIX: Si el track est├í oculto por una carpeta, sus clips NO TIENEN
  // COORDENADA Y (deber├¡an ocultarse)
...
void PlaylistComponent::paint(juce::Graphics &g) {
  ...
  // --- OPTIMIZACI├ôN DE RENDIMIENTO: CACH├ë DE COORDENADAS Y CULLING ---
  // 1. Pre-calculamos las alturas Y de todas las pistas para evitar 5,000+
  // b├║squedas redundantes
  ...
  // 2. L├¡mites visibles para Culling Horizontal inteligente
  ...
  if (tracksRef) {
    for (auto *t : *tracksRef) {
      if (!t->isShowingInChildren)
        continue;
      g.setColour(
          juce::Colours::black.withAlpha(0.8f)); // Separador mucho m├ís oscuro
      g.fillRect(0.0f, (float)(currentY + (int)trackHeight - 2),
                 (float)getWidth(), 2.0f); // 2 p├¡xeles de grosor puro
      currentY += (int)trackHeight;
    }
  }

  // --- NUEVO: RENDERIZADO DE RES├ÜMENES DE CARPETA (REAPER STYLE) ---
  if (tracksRef) {
    for (auto *t : *tracksRef) {
      if (t->getType() == TrackType::Folder) {
        ...
        // Culling vertical b├ísico para el resumen de la carpeta
        ...
        // 1. Calcular rango global de notas para la carpeta (Alineaci├│n
        // Vertical)
        ...
        // 2. Dibujar res├║menes de clips que pertenezcan a esos descendientes
  ...
  for (int i = 0; i < (int)clips.size(); ++i) {
    ...
    // BUG FIX: Si el track est├í oculto por una carpeta, yPos ser├í -1. Ignoramos
    // este clip.
    if (yPos == -1)
      continue;
    ...
    // Culling vertical correcto: omitimos si el clip completo est├í por encima o
    ...
    // 1. Dibujar el Clip
    if (clip.linkedAudio != nullptr) {
        // --- L├ôGICA DE AUDIO (Shell est├índar) ---
        ...
        // --- L├ôGICA DE MIDI (Delegaci├│n total al Renderer basado en Estilos) ---
    ...
    // 4. Indicador de selecci├│n (Universal)
  ...
  if (phX >= 0 && phX <= getWidth()) {
    ...
    // 2. L├¡nea principal (Blanca)
    g.setColour(juce::Colours::white.withAlpha(0.85f));
    g.drawVerticalLine(phX, phTop, phBottom);

    // 3. Tri├íngulo invertido en el timeline (sim├®trico)
    juce::Path triangle;
    float triTopY = phTop;
